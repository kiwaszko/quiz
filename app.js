...
const MIN_REWARD_SCORE = 75; // score % threshold to show reward panel

// Quiz schedule days: 1=Mon, 2=Tue, 3=Wed, 4=Thu, 5=Fri, 6=Sat, 7=Sun
const QUIZ_DAYS = [3, 5];
...
const DOM = {
  ...
  // Footer username badges (one per screen)
...
const getCurrentDayOfWeek = () => {
  const d = new Date();
  return d.getDay() === 0 ? 7 : d.getDay(); // Sun=0→7, Mon=1…Sat=6
};
...
const startCountdown = (el, callback) => {
  ...
  const iv = setInterval(() => {
    ...
    if (remaining !== lastSec) { el.textContent = remaining; triggerPulse(el); lastSec = remaining; }
...
const refreshUsernameUI = () => {
  ...
  // Already-done screen footer badge
  if (name) {
...
// ════════════════════════════════════════════════════════════════════
// REWARD SEND HANDLER (first-time entry in reward block)
// ════════════════════════════════════════════════════════════════════
const handleRewardSend = async () => {
  ...
  try {
    ...
    // Hide the reward input panel after short delay
    setTimeout(() => {
      DOM.rewardContainer.classList.add("hidden");
    }, 1500);
  } catch (err) {
    // GAS failed – do NOT save to localStorage; show error
...
const handleEditSave = async () => {
  ...
  try {
    await sendUsernameToGAS(username);
    // Only save to localStorage after GAS success
    storeUsername(username);
    refreshUsernameUI();
...
const displayCompletedQuiz = async () => {
  ...
  // Reward panel: only if score qualifies AND username not yet stored
  DOM.rewardContainer.classList.toggle("hidden", !(qualifiesReward && !hasUsername));

  // No-reward info: only if score does NOT qualify
  if (!qualifiesReward) {
...
const handleSubmit = async () => {
  ...
  // Include username in payload so GAS can return it / store it
  const payload = [
    `action=submitQuiz`,
  ...
  try {
    ...
    // If GAS returned a username back (meaning it was stored server-side),
    // make sure we have it in localStorage too
...
const init = async () => {
  ...
  // 2. Not a quiz day — show last quiz if recent enough
...
// Footer username badge → open edit modal
